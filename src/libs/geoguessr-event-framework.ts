...
export class GEF {
  ...
  constructor() {
    ...
    THE_WINDOW.addEventListener("load", () => {
      if (location.pathname.startsWith("/challenge/")) {
        ...
        THE_WINDOW.GEFFetchEvents.dispatchEvent(
          new CustomEvent("received_data", { detail: data })
        )
      }
    })
  ...
  private overrideFetch(): void {
    ...
    THE_WINDOW.fetch = (function () {
      return async function (...args) {
        ...
        if (
          /geoguessr.com\/api\/v3\/(games|challenges)\//.test(url) &&
          url.indexOf("daily-challenge") === -1
        ) {
  ...
  private loadState(): void {
    const data = window.localStorage.getItem("GeoGuessrEventFramework_STATE")
    if (!data) return

    const dataJson: GEF_State = JSON.parse(data)
    if (!dataJson) return
  ...
  private startRound(data: any): void {
    ...
    if (this.state.current_round === 1) {
      this.events.dispatchEvent(
        new CustomEvent("game_start", { detail: this.state })
      )
    }

    this.events.dispatchEvent(
      new CustomEvent("round_start", { detail: this.state })
    )
  }
  ...
  private stopRound(data: any): void {
    ...
    this.events.dispatchEvent(
      new CustomEvent("round_end", { detail: this.state })
    )

    if (this.state.current_round === 5) {
      this.events.dispatchEvent(
        new CustomEvent("game_end", { detail: this.state })
      )
    }
  }
}
...
//             meters: {
//               amount:
//                 parseFloat(data?.player?.totalDistance?.meters?.amount) || 0,
//               unit: data?.player?.totalDistance?.meters?.unit || "km"
//             },
//             miles: {
//               amount: parseFloat(data?.player?.totalDistance?.miles?.amount) || 0,
