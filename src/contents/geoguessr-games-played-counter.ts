...
import { GM_addStyle } from "~src/libs/gm"
...
ref: https://github.com/miraclewhips/geoguessr-userscripts/blob/master/geoguessr-games-played-counter.user.js

- EventFrameworkとStreakFrameworkは共通
- unsafeWindowを使っているので、windowを使うように変更
- GM_addStyleは自前のやつに変更。引数にdocumentが追加されているので注意
- __NEXT_DATA__ はEventFrameworkがwindowの下に生やしているので window.__NEXT_DATA__ で取得
...
function gameValue() {
  switch (STATE.type) {
    case "counter":
      return `${(STATE.games_played - STATE.games_conf_val).toLocaleString()}`
    case "target":
      return (STATE.games_conf_val - STATE.games_played).toLocaleString()
    default:
      return STATE.games_played.toLocaleString()
  }
}
...
function updateSummaryPanel() {
  ...
  const scoreLayoutBottom = document.querySelector(
...
async function init() {
  ...
  const USER_ID =
    window["__NEXT_DATA__"]?.props?.accountProps?.account?.user?.userId
  if (!USER_ID) return
...
function updateFormDisplay() {
  ...
  switch (vals.type) {
    case "counter":
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      //@ts-ignore
      input.placeholder = "Starting count #"
      break
    case "target":
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      //@ts-ignore
...
function configure() {
  ...
  document
    .getElementById("type-total")
    .addEventListener("change", updateFormDisplay)
  document
    .getElementById("type-counter")
    .addEventListener("change", updateFormDisplay)
  document
    .getElementById("type-target")
    .addEventListener("change", updateFormDisplay)

  document
    .getElementById("mw-gpc-conf-form")
