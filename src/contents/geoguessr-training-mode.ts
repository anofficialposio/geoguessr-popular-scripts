...
import { GM_addStyle, GM_openInTab } from "~src/libs/gm"
...
ref: https://github.com/miraclewhips/geoguessr-userscripts/blob/master/geoguessr-training-mode.user.js
...
// -- I added this cuz I know what I'm doing

const GeoGuessrEventFramework = new GEF()
console.log(
...
function loadState() {
  const data = window.localStorage.getItem("mwgtm_state")
  if (!data) return
...
const MWGTM_STATE = defaultState()
loadState()
...
async function mmaFetch(url, options = {}) {
  ...
  if (!response.ok) {
    ...
    alert(
      `An error occurred while trying to connect to Map Making App. ${message}`
    )
...
if (!GeoGuessrEventFramework) {
  throw new Error(
    "GeoGuessr Location Manager requires GeoGuessr Event Framework (https://github.com/miraclewhips/geoguessr-event-framework). Please include this before you include GeoGuessr Location Manager."
  )
}
...
function addSettingsButtonsToRound() {
  ...
  const settings = JSON.parse(window.localStorage.getItem("game-settings")) ?? {
...
function createSettingsButtonEvents() {
  if (document.getElementById("mwgtm-opt-compass-north")) {
    document
      .getElementById("mwgtm-opt-compass-north")
  ...
  document
    .getElementById("mwgtm-opt-compass-toggle")
    .addEventListener("click", () => {
      toggleCompass(undefined)
    })
  ...
  document
    .getElementById("mwgtm-opt-coverage")
    .addEventListener("click", () => {
      toggleCoverage(undefined)
    })
...
function toggleCompass(hidden) {
  ...
  document.getElementById("mwgtm-opt-compass-toggle").textContent = hidden
    ? "COMPASS HIDDEN - [ H ]"
    : "COMPASS VISIBLE - [ H ]"
...
function toggleTerrain(enabled) {
  ...
  document.getElementById("mwgtm-opt-terrain").textContent = enabled
    ? "TERRAIN ENABLED - [ T ]"
    : "TERRAIN DISABLED - [ T ]"

  if (document.getElementById("mwgtm-opt-terrain-summary")) {
    document.getElementById("mwgtm-opt-terrain-summary").textContent = enabled
      ? "TERRAIN ENABLED - [ T ]"
      : "TERRAIN DISABLED - [ T ]"
  }
...
function toggleCoverage(enabled) {
  ...
  document.getElementById("mwgtm-opt-coverage").textContent = enabled
    ? "COVERAGE VISIBLE - [ B ]"
    : "COVERAGE HIDDEN - [ B ]"

  if (document.getElementById("mwgtm-opt-coverage-summary")) {
    document.getElementById("mwgtm-opt-coverage-summary").textContent = enabled
      ? "COVERAGE VISIBLE - [ B ]"
      : "COVERAGE HIDDEN - [ B ]"
  }
...
async function clickedMapButton(e) {
  if (MAP_MAKING_API_KEY === "PASTE_YOUR_KEY_HERE") {
    alert(
      "An API Key is required in order to save locations to Map Making App. Please add your API key by editing the Userscript and following the instructions at the top of the script."
    )
    return
  }
...
function showMapList() {
  ...
  if (MWGTM_STATE.recentMaps.length > 0) {
    let recentMapsHTML = ""
    for (const m of MWGTM_STATE.recentMaps) {
...
function addLocationToMap(e) {
  ...
  MWGTM_STATE.recentMaps = MWGTM_STATE.recentMaps
    .filter((e) => e.id !== id)
    .slice(0, 2)
...
function addSettingsButtonsToSummary() {
  ...
  if (!container || document.getElementById("mwgtm-settings-buttons-summary"))
    return
...
function createSettingsButtonSummaryEvents() {
  document
    .getElementById("mwgtm-opt-save-loc")
    .addEventListener("click", () => {
      clickedMapButton(undefined)
    })

  document
    .getElementById("mwgtm-opt-open-maps")
    .addEventListener("click", () => {
      const link = googleMapsLink(LOCATION)
      GM_openInTab(window, link, false)
    })

  document
    .getElementById("mwgtm-opt-terrain-summary")
    .addEventListener("click", () => {
      toggleTerrain(undefined)
    })

  document
    .getElementById("mwgtm-opt-coverage-summary")
    .addEventListener("click", () => {
      toggleCoverage(undefined)
    })
}
...
const observer = new MutationObserver(() => {
  ...
  const controls =
    document.querySelector(`aside[class^="game_controls__"]`) ||
    document.querySelector(`aside[class^="game-panorama_controls__"]`)
  if (!controls) return
      ...
      constructor(...args) {
        ...
        MWGTM_SVC = new google.maps.ImageMapType({
          getTileUrl: (point, zoom) =>
            `https://www.google.com/maps/vt?pb=!1m7!8m6!1m3!1i${zoom}!2i${point.x}!3i${point.y}!2i9!3x1!2m8!1e2!2ssvv!4m2!1scc!2s*211m3*211e2*212b1*213e2*212b1*214b1!4m2!1ssvl!2s*211b0*212b1!3m8!2sen!3sus!5e1105!12m4!1e68!2m2!1sset!2sRoadmap!4e0!5m4!1e0!8m2!1e1!1e1!6m6!1e12!2i2!11e0!39b0!44e0!50e`,
...
function installGetContext(el) {
  ...
  el.getContext = function () {
    ...
    if (
      (arguments[0] === "webgl" || arguments[0] === "webgl2") &&
      ctx &&
      ctx.shaderSource &&
